...
/* global MQTT_CLIENT */
const DEVICE_TRANSPORT = process.env.DEVICES_TRANSPORT || 'HTTP';
...
if (DEVICE_TRANSPORT === 'HTTP') {
    debug('Listening on HTTP endpoints: /iot/test'); 
    ...
    iotRouter.post('/iot/test:id', Southbound.testHttpCommand);//edit
    //iotRouter.post('/iot/door:id', Southbound.doorHttpCommand);//edit
    //iotRouter.post('/iot/lamp:id', Southbound.lampHttpCommand);//edit
    // Dummy ISOBUS/ISOXML endpoint.
    iotRouter.post('/iot/isoxml', Southbound.isobusHttpCommand);//edit
...
//     const apiKeys = process.env.DEVICES_API_KEYS || process.env.DEVICES_API_KEY || '1234';
