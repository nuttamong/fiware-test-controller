...
const DEVICE_API_KEY = process.env.DEVICES_API_KEY || '1234';

const IOT_AGENT_URL =
    'http://' +
    (process.env.IOTA_HTTP_HOST || '10.101.118.213') +
    ':' +
    (process.env.IOTA_HTTP_PORT || 7896) +
    (process.env.IOTA_DEFAULT_RESOURCE || '/iot/json');

function getAPIKey(deviceId) {
    return process.env.DEVICES_API_KEY ? DEVICE_API_KEY : hashCode(deviceId.replace(/[0-9]/gi, ''));
}
